...
export default function Link() {
    ...
    return (
        <div>
            <Select label="Link type" onChange={updateLinkType}>
                <Option value='' selected={linkType === ''}>Choose</Option>
                <Option value={LINK_TYPES.URL} selected={linkType === LINK_TYPES.URL}>
                    <Icon className={style.optionIcon} type="material" name="link"/> Regular link
                </Option>
                ...
                <Option value={LINK_TYPES.TEL} selected={linkType === LINK_TYPES.TEL}>
                    <Icon className={style.optionIcon} type="material" name="call"/> Phone number
                </Option>
                ...
                <Option value={LINK_TYPES.ANCHOR} selected={linkType === LINK_TYPES.ANCHOR}>
                    <Icon className={style.optionIcon} type="material" name="tag"/> Element on the page
                </Option>
            </Select>
            ...
            {linkType
                ? <div>
                    {linkType === LINK_TYPES.ANCHOR
                        ? <Select label="Select an anchor" onChange={updateLinkValue}>
                            <Option value="" selected={linkValue === ''}>Choose</Option>
                            {allAnchors.map(anchor =>
                                <Option value={`#${anchor}`} key={anchor}
                                        selected={linkValue === `#${anchor}`}>#{anchor}</Option>)}
                        </Select>

                        : <Input onChange={(ev, val) => updateLinkValue(val)} value={linkValue} label="Enter a value"/>
                    }

                    <Select label="Open in:" onChange={updateLinkTarget}>
                        <Option value="" selected={linkTarget === ''}>Choose</Option>
                        <Option value="_self" selected={linkTarget === '_self'}>The same card</Option>
                        <Option value="_blank" selected={linkTarget === '_blank'}>New tab</Option>
                    </Select>
                </div> : <></>}
